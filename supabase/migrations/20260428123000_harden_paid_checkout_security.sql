-- Harden paid checkout RPCs and webhook job dedupe.

update public.payment_jobs duplicate
set status = 'failed',
    last_error = 'Duplicate webhook payment job suppressed before unique index.',
    locked_at = null,
    updated_at = now()
from (
  select id
...
create unique index if not exists payment_jobs_payment_event_active_unique
  on public.payment_jobs (payment_event_id)
  where payment_event_id is not null
    and status <> 'failed';
...
revoke execute on function public.check_checkout_rate_limit(text, text, integer, integer) from public, anon, authenticated;
revoke execute on function public.lock_checkout_capacity_buckets(uuid) from public, anon, authenticated;
revoke execute on function public.reserve_booking_capacity(uuid, integer) from public, anon, authenticated;
revoke execute on function public.prepare_checkout_payment_attempt(uuid, integer, integer) from public, anon, authenticated;
revoke execute on function public.fulfill_booking_intent(uuid, uuid, text[]) from public, anon, authenticated;
revoke execute on function public.claim_payment_jobs(integer, integer) from public, anon, authenticated;
revoke execute on function public.release_expired_booking_holds() from public, anon, authenticated;

grant execute on function public.check_checkout_rate_limit(text, text, integer, integer) to service_role;
grant execute on function public.lock_checkout_capacity_buckets(uuid) to service_role;
grant execute on function public.reserve_booking_capacity(uuid, integer) to service_role;
grant execute on function public.prepare_checkout_payment_attempt(uuid, integer, integer) to service_role;
grant execute on function public.fulfill_booking_intent(uuid, uuid, text[]) to service_role;
grant execute on function public.claim_payment_jobs(integer, integer) to service_role;
